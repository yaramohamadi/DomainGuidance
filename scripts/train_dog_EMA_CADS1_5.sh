...
# This script trains the DiT model on the Caltech-101 dataset.
  ...
  CUDA_VISIBLE_DEVICES=1,3 torchrun --nproc_per_node=2 train_with_guidance_EMA.py \
  --data-path "$TARGET_DIR/caltech-101/" \
  --results-dir /export/livia/home/vision/Ymohammadi/DoG/results_dogfinetune1_5_EMA_CADS/ \
  --model DiT-XL/2 \
  --image-size 256 \
  --num-classes 102 \
  --total-steps 24000 \
